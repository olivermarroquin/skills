...
> **v1.2 changelog (2026-06-12, [WF-2] close-out)** — Cross-link to the sibling `seo-site-teardown` skill (v1.1, registered 2026-06-11). Boundary made explicit in "Do not use": this skill is the **landscape** tier (who ranks, side-by-side briefs + synthesis); for a full-build reproduction teardown of ONE site you intend to out-build, hand off to `seo-site-teardown`. Compose them: landscape first, then teardown the winner. No behavior change.
...
- A full-build reproduction teardown of a single site (page-tier taxonomy, per-entity data layer, schema graph, complete tech/tools stack, conversion system, what actually ranks) — that's the deeper sibling **`seo-site-teardown`** (`~/workspace/skills/seo-site-teardown/`). Compose them: run this skill first for the landscape, then `seo-site-teardown` on the one site you intend to out-build. It's the mandatory first step of every website-factory client build.
