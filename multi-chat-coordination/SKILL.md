...
**Phase 3 orchestrator (shipped 2026-06-01).** The `vault-orchestrator` skill (at `~/workspace/skills/vault-orchestrator/SKILL.md`) sits one layer above this skill. Two modes: SURVEY (state-of-the-vault report in 9 ordered sections — in-flight / ready / queued / decisions / scheduled / recent wins / domain signals / stale signals / cross-project signals) and NEXT-MOVES (ranked spawnable candidates + session plan capped at 8 hours). NEXT-MOVES **composes with this skill's Mode 3 (NEXT-MOVE)** to get the base leverage ranking, then layers four overlay signals (unblocks-downstream-count, time-to-revenue, operator-stated-priority cross-check, recency-of-related-work) plus session-budget totals (neutral display, no editorializing), parallel-work detection, serial-blocked detection, per-candidate substrate recommendation per the working-surfaces convention, and decision-research convention on ties. The orchestrator is read-only on vault content; it produces reports, never writes to per-project files or to the master tracker outside the aggregator's marker block. See `~/workspace/skills/vault-orchestrator/references/composition-with-multi-chat-coordination.md` for the exact handoff shape.
...
**`~/workspace` is NOT a single git repo.** It contains nested repos. The most common ones a chat will touch: `~/workspace/second-brain/` (vault edits — handoffs, execution logs, conventions, primers, source notes, tactics, tools, patterns, READMEs), `~/workspace/skills/` (skill files), `~/workspace/repos/ai-agency-core/` (shared scripts + data files), `~/workspace/repos/<client-slug>/` (per-client repos). Closing Protocol Step 5 identifies which repos got touched; Step 6 produces one separate git command block per repo. Mixing files from multiple nested repos into one `git add` doesn't work.

**Every git command block opens with `rm -f .git/index.lock`.** Cowork sessions or background tools often leave a stale `.git/index.lock` that blocks subsequent `git add` / `git commit` with `fatal: Unable to create '...index.lock': File exists`. The `rm -f` flag means "no error if the file doesn't exist" — so the line is a safe defensive no-op when no lock is present, and a recovery step when one is. The operator paste experience is uninterrupted either way. The convention is documented in `references/closing-protocol-template.md` Step 6 and propagates through every DECOMPOSE-generated handoff.

**Every closing message ends with an explicit "What's next to spawn" line.** Operator-requested 2026-05-28 after the intel-routing Phase 3 chat shipped Session 1 with "Session 2 (PULL) ready to spawn next. Phase 3 handoff stays `status: active` until Session 3 ships per the per-session protocol." That kind of line is what the operator actually reads + acts on at the end of a chat. The pattern is mandatory in `references/closing-protocol-template.md` Step 7 — every chat must name (1) the specific next chat, session, or move with its path/handoff reference, OR (2) "nothing immediately next" plus a reference to current Ready-to-spawn candidates, OR (3) the specific manual action blocking the next chat if applicable. No vague "next session" — name it concretely.
