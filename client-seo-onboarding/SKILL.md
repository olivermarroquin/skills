...
- **Website-factory path check (v1.7).** If the engagement includes — or is expected to graduate to — a custom-site build (website-factory program), confirm a `site-capture-engine` run exists for the client's strongest reference competitor (look for `<client-vault>/admin-extracts/competitor-research/<slug>-teardown/`). If absent, surface to the operator: "This client is on the custom-build path; the website-factory program requires a `site-capture-engine` of the strongest reference site as the mandatory first step. Run it now (~6–12h, decomposable by pass) or schedule it as a parallel chat?" The teardown's reproduction blueprint is what the build phases consume; the Core 30 pipeline below does not block on it. Compose: `competitor-deep-research` for the landscape → `site-capture-engine` on the one site to out-build.
