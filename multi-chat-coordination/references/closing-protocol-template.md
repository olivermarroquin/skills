...
- **The independent peer-review chat AUTHORS the run's rows — not the producer.** The reviewer is the only party that can see what the producer's gate and quality-loop *missed*, so it grades them honestly (a producer grading its own review is the same self-gate bias this system rejects — and real-run evidence 2026-06-18 confirms producer self-grades only land honest *because* the reviewer's catches force them). At the end of its review (after PASS), the reviewer appends **all** rows for the run to `~/workspace/second-brain/_meta/handoffs/_review-skill-firing-tracker.md` — **one row per (run × review skill)** for `Gate-blocking reviewer`, `Peer-review`, `Quality-control`:
  - Its **own** `Peer-review` row (Role = `Reviewer`) — what it caught that the producer's layers missed.
  - The producer's `Gate-blocking reviewer` + `Quality-control` rows (Role = `Producer`) — the **producer supplies the facts** (did the Stop hook fire / `Blocked-me`? was `output-quality-loop` invoked? any exemption); the **reviewer assigns the grade + worth-keeping verdict** based on what those layers caught vs. what the reviewer had to catch.
  - For each row: Fired? (`Yes`/`No`/`Exempt`/`Blocked-me`; `Auto-skipped` — the circuit breaker shipped 2026-06-18) + why (a silent skip is a defect — name the exemption), what it caught (count + severity + catch IDs), Grade (single token `A`–`F`, `N/A` if exempt), Worth keeping? (`Keep`/`Drop`/`Conditional`/`N/A`), what would make it worth keeping. Use this chat's event-log Run ID so all rows group.
  - **No paired reviewer** (genuinely exempt run, or a Cowork chat with no reviewer spawned): the producer self-reports its rows and marks them `self-reported (no independent grader)` in the Why column — the weak data point, flagged as such.
