...
| Code artifact (executable source) | **4** | top 3 | Added v1.4 (2026-06-18). Low cap — most code quality is verifiable on disk (build / tests / run-it), not researched. Queries target only unfamiliar framework idioms (e.g. an ORM's idempotent-upsert pattern) or a security boundary on a gate artifact (known bypass classes). The `gate-peer-reviewer` independent-reviewer mandate governs adversarial probing; this cap covers the rare external-lookup gap. |
